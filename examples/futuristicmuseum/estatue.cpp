...
void Estatue::loadDiffuseTexture(std::string_view path) {
...
void Estatue::loadNormalTexture(std::string_view path) {
  if (!std::filesystem::exists(path))
    return;
...
void Estatue::loadObj(std::string_view path) {
...
void Estatue::render(int numTriangles) const {
...
void Estatue::setupVAO(GLuint program) {
...
void Estatue::create(GLuint program, ObjectConfiguration configuration) {
  ...
  normalTexturepath = configuration.normalTexturepath;
  difuseTexturepath = configuration.difuseTexturepath;
  m_mappingMode = configuration.mode;
  // Variaveis uniformes
  ...
  m_lightDir = glm::vec4{startPosition.x, -0.7, startPosition.y, -0.5};
  // m_Id = glm::vec4{color, 1};
  // m_Is = glm::vec4{color, 1};
  loadDiffuseTexture(difuseTexturepath);
  loadNormalTexture(normalTexturepath);
  loadObj(path);
  setupVAO(m_program);
...
void Estatue::paint(glm::mat4 m_viewMatrix) {
